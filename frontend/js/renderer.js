...
async function initWebGPU() {
  if (!navigator.gpu) {
    console.warn('[Renderer] WebGPU not available in this browser — staying on Canvas 2D path');
    return false;
  }

  const adapter = await navigator.gpu.requestAdapter();
  if (!adapter) {
    console.warn('[Renderer] No WebGPU adapter found — staying on Canvas 2D path');
    return false;
  }
  ...
  gpuContext = canvas.getContext('webgpu');
    if (!gpuContext) {
        console.warn('[Renderer] Failed to get WebGPU context — staying on Canvas 2D path');
        return false;
    }
...
async function getEntityRenderer(entityId, animationDataPaths) {
    ...
    if (!entityRenderers.has(entityId)) {
        const renderer = new EntityRenderer(ctx, entityId, animationDataPaths, useGPU);
