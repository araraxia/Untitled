...
| `localOffset` | `object \| null` | `{"position": [x,y,z], "rotation": [x,y,z], "scale": [x,y,z], "origin"?: [x,y,z]}`, default identity (`origin` defaults to `[0,0,0]`, i.e. the mesh's own local origin — most parts never set it). **Deliberately not called `transform3d`** — `localOffset` is definition-level (the fixed relationship between two meshes of the same asset, true for every placed instance); `transform3d` (on the networked entity, not here) is per-instance. Conflating the two names would make it easy to confuse "adjust this asset's internal composition" with "adjust where this one placement is". `origin` is a rotation/scale pivot point in the part's own local space — `position` still translates normally, but rotation and scale happen around `origin` instead of `[0,0,0]`, via `mat4.compose_with_pivot()` (see [MAT4.md](MAT4.md)) — useful when a mesh's own authored origin (wherever Blender happened to put it) doesn't coincide with where it should actually pivot (a wing hinging at its socket, not its mesh center). |
...
| `keyframes` | `array` | `{"time_ms": number, "position"?: [x,y,z], "rotation"?: [x,y,z], "scale"?: [x,y,z], "easing"?: [x1,y1,x2,y2]}`, sorted ascending by `time_ms`. Any subset of the three transform fields per keyframe — omitted fields hold the part's `localOffset` rest value (see the `animation_id` row above), not `[0,0,0]`/`[1,1,1]`. `easing` is an optional cubic-bezier control-point pair (the same `cubic-bezier(x1,y1,x2,y2)` convention CSS/After Effects use) governing the transition *into* that keyframe — default `[0,0,1,1]`, a straight line, i.e. plain linear interpolation, identical to every clip authored before this field existed. See `client/engine/transform_clip.py`'s module docstring (`cubic_bezier_ease`/`resolve_easing`) for the full sampling algorithm — a multi-part rig clip's own per-keyframe `"parts"` map entries (below) carry this same optional field, per-part. |
| `origins` | `object \| null` | Optional `{part_id: [x,y,z]}` map, for a multi-part rig clip (see `client/engine/entity_builder.py`'s Animation Editor) — a rotation/scale pivot point for that part, constant for the whole clip (not per-keyframe, since a hinge/pivot doesn't move keyframe to keyframe). Sampled straight through, unmodified, as `sample_transform_clip()`'s `"origin"` result field whenever the sampling `part_id` matches — same `localOffset.origin` meaning and mechanism above, just sourced from the clip instead of the part's own static fields. |
